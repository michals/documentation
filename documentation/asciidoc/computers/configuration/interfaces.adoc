...
| `UART0` through to `UART4` and `UART10` (also called the debug UART): PL011 (disabled by default) +
  No mini UART
|===
...
The *primary UART* is the main serial interface that a Raspberry Pi exposes. This is the UART that you typically use when you connect external devices and for a Linux serial console.

For all Raspberry Pi devices other than Raspberry Pi 5, the primary UART is routed to specific GPIO pins:

* *GPIO 14* (transmit) is pin 8 on the GPIO header.
* *GPIO 15* (receive) is pin 10 on the GPIO header.

On Raspberry Pi 5, by default the primary UART is exposed through the dedicated debug header instead (`UART10`). This dedicated debug header is a three-pin header labelled `UART`. However, the following `config.txt` settings can change the primary UART:

- If xref:../computers/config_txt.adoc#enable_rp1_uart[`enable_rp1_uart`] is set to 1, Raspberry Pi 5 routes later firmware debug messages, including all file accesses, through the UART on GPIO 14 and 15.
- If there is no UART cable plugged into the dedicated debug header and xref:../computers/config_txt.adoc#enable_uart[`enable_uart`] is set to 1, Raspberry Pi 5 automatically routes kernel logging debug output to the UART on GPIO 14 and 15.
...
.Primary and secondary UARTs
|===
| Model | Primary (Linux console) | Secondary (Bluetooth on Raspberry Pi devices with wireless)
